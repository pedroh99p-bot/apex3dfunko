...
await suite('mvp-smoke', 4175, async ({ page, run, open, field, inspect, total, upload, fixture, complete, review, generate, extras, go }) => {
  ...
  await run('Combinação: pessoa, pet, acessórios, objeto, base e caixa', async () => {
    ...
    await page.locator('[data-mf-card-kind=extras] [data-mf-extras-tab=accessories]').first().click();
    await page.locator('[data-apex-additional-category=detalhado]').first().click();
  ...
  for (const width of [1440,1024,768,430,390,375,360]) {
    await run('Upsells e revisão responsivos em ' + width + 'px', async () => {
      ...
      await extras(); await page.locator('#personalizar').scrollIntoViewIfNeeded();
      assert.ok(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth), JSON.stringify(await page.evaluate(()=>({w:document.documentElement.scrollWidth,culprits:[...document.querySelectorAll('body *')].filter(n=>{const r=n.getBoundingClientRect();if(!r.width||r.right<=innerWidth)return false;let p=n.parentElement;while(p&&p!==document.body){if(['hidden','auto','scroll','clip'].includes(getComputedStyle(p).overflowX))return false;p=p.parentElement;}return true;}).slice(0,12).map(n=>({tag:n.tagName,parent:n.parentElement.className,html:n.outerHTML.slice(0,220),cls:typeof n.className==='string'?n.className:'svg',right:n.getBoundingClientRect().right}))}))));
