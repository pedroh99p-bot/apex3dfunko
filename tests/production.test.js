...
test('quatro produtos geram draft BRL validado sem características manuais', async t => {
  for (const [type, expected] of [['individual',9990],['pet',9990],['casal',18990],['familia',26990]]) {
    ...
    assert.equal(result.orderDraft.productionValidation, 'passed'); assert.equal(result.orderDraft.productionReady, false); assert.equal(Object.hasOwn(result.orderDraft.pricing, 'status'), false); assert.equal(result.orderDraft.shipping.date, '2030-05-10');
...
test('pessoa adicional exige sua própria foto', async t => {
  ...
  assert.equal(createOrderDraft(state, options).orderDraft.pricing.totalCents, 19980);
});
