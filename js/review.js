...
export function renderOrderReview(container, state, { uploads = [], onEdit, onGenerate }) {
  ...
  c.figures.forEach((f, i) => {
    ...
    section('Pessoa ' + (i + 1), [
      ...
      ['Detalhes de aparência', f.fields.appearance || 'Conforme as fotos'],
      ['Óculos', f.glasses ? 'Incluir conforme referência' : 'Conforme as fotos'],
    ...
    ], [...reference, ...photos(f.id + '.mf_outfit_photo_upload[]'), ...photos(f.id + '.mf_face_glasses_upload')]);
  });
  ...
  if (c.box.type !== 'caja_standard') {
    ...
    if(c.box.fields.mf_box_number) packRows.push(['Número na caixa', c.box.fields.mf_box_number]);
    if(c.box.fields.mf_box_collection_name) packRows.push(['Coleção', c.box.fields.mf_box_collection_name]);
    if(c.box.fields.mf_box_color) packRows.push(['Cor da caixa', c.box.fields.mf_box_color === 'custom' ? (c.box.fields.mf_box_color_custom || 'A confirmar') : c.box.fields.mf_box_color]);
